...
// Possible new syntax with flag and arg processing support

// command('log [*when] [-flags]', {
...
//   ],
//   flags: [
//     [['-s', '--summary'], function(options) {
//       options.showSummary = true
//       return options
//     }]
//   ],
//   run: function(args, options) {
//     // The command's main stuff
//   }
// })
...
command('edit <date> [editor]',
        'edit punchfile for the given date - uses EDITOR env var unless and editor command is specified',
        (args) => {

  const editor = args.editor || process.env.EDITOR
  const date = moment(args.date, 'MM/DD/YYYY').startOf('day')
  const fs = require('fs')
