...
module.exports = class B2SyncService extends SyncService {
  ...
  async upload (uploads = [], manifest = {}) {
    ...
    try {
      uploaded = await Promise.all(uploads.map(async punch => {
...
function B2Credentials (credentials, appConfig) {
  ...
  if (typeof credentials === 'string') {
    ...
    try {
      file = fs.readFileSync(resolvePath(credentials, path.dirname(appConfig.configPath)), 'utf8')
    } catch (err) {
      throw err
    }
    ...
    case '.json':
      ...
      break
    default:
      throw new Error(`${ext} files are not supported for credentials. Use .mon or .json`)
    }
  }

  // Check if properties are there
  if (!credentials.accountId || !credentials.applicationKey) {
    throw new Error('B2 credentials must have both accountId and applicationKey properties')
  }
