...
describe("duration", () => {
  it("formats Duration into a duration string", () => {
    expect(
      formatDuration(fromParts({ hours: 3, minutes: 30, seconds: 45 }))
    ).toBe("3h 30m 45s");
  });

  it("omits hours if less than one hour", () => {
    expect(formatDuration(fromParts({ minutes: 26, seconds: 15 }))).toBe(
      "26m 15s"
    );
  });
  ...
  it("displays minutes even if minutes is 0 when longer than one hour", () => {
    expect(formatDuration(fromParts({ hours: 52, seconds: 7 }))).toBe(
      "52h 0m 7s"
    );
  });
  ...
  describe("resolutions", () => {
    ...
    it("returns the expected result if resolution is set to minutes", () => {
      expect(formatDuration(duration1, { resolution: "minutes" })).toBe(
        "5h 13m"
      );
      expect(formatDuration(duration2, { resolution: "minutes" })).toBe("58m");
      expect(formatDuration(duration3, { resolution: "minutes" })).toBe(
        "15h 33m"
      );
    });

    it("returns the expected result if resolution is set to seconds", () => {
      expect(formatDuration(duration1, { resolution: "seconds" })).toBe(
        "5h 13m 12s"
      );
      expect(formatDuration(duration2, { resolution: "seconds" })).toBe(
        "58m 16s"
      );
      expect(formatDuration(duration3, { resolution: "seconds" })).toBe(
        "15h 32m 58s"
      );
    });

    it("returns the expected result if resolution is set to milliseconds", () => {
      expect(formatDuration(duration1, { resolution: "milliseconds" })).toBe(
        "5h 13m 12s 124ms"
      );
      expect(formatDuration(duration2, { resolution: "milliseconds" })).toBe(
        "58m 15s 662ms"
      );
      expect(formatDuration(duration3, { resolution: "milliseconds" })).toBe(
        "15h 32m 58s 10ms"
      );
    });
  });
