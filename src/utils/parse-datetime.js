...
function parseTime(timeString) {
  ...
  if (timePattern.test(timeString.trim())) {
    ...
    throw new Error(
      "Time is not in a compatible format: use either 12-hour with AM/PM or 24-hour format for time"
    );
  }
  ...
  if (meridian) {
    if (hours > 12) {
      throw new Error(
        "Hours should not be greater than 12 when AM/PM is specified."
      );
    } else {
  ...
  if (str.indexOf("@") === -1) {
    if (timePattern.test(str.trim())) {
      ...
      if (parsed) {
        ...
        throw new Error(
          "Datetime string should be in the format DATE@TIME[AM/PM]"
        );
      }
    }
  }
