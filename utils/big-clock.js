...
function getTransFrameInfo (line) {
  ...
  const [, from, to, timing] = line.match(/--:\s*(.+?)\s*>\s*(.+?)\s*\[(\d+)\]\s*:--/)
...
module.exports = function ({ style, letterSpacing, colors, animate }) {
  ...
  try {
    const clockPath = path.join(global.appRoot, 'resources', 'clock-styles', style + '.clock')
  ...
  for (const char in clock.characters) {
    clock.characters[char] = colorizeChar(clock.characters[char], spaceChar, colors)
  }

  if (animate) {
    for (const from in clock.transitions) {
      for (const to in clock.transitions[from]) {
        clock.transitions[from][to] = clock.transitions[from][to]
          .map(char => ({
            ...char,
